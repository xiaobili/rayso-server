...
const server = createServer((request, response) => {
	...
	(async () => {
		...
		page.target().createCDPSession().then(session => {
		...
		await page.goto(`http://localhost/#theme=${path.theme}&background=true&darkMode=true&padding=64&code=${path.code}`);


		const linkHandlers = await page.$x("/html/body/div/div/div[3]/div[6]/button[1]");
		...
		if (filename) {
			...
			stream.on("end", () => {
				rename("./cache/" + filename, "./cache/" + randomUUID() + ".png", (err) => {
					if (err) {
						console.log(err);
					}
				}
